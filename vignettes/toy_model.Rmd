...
output_name <- aa$output

aa$simulations_multi %>%
  map(~ select(., matches(output_name))) %>%
  map(~ set_names("x")) %>%
  map()

my_sims <- aa$simulations_multi %>%
  imap_dfr(~ mutate(.x, sim = .y)) %>%
  select(sim, matches(output_name)) %>%
  set_names(c("sim", "x"))

p1 <- my_sims %>%
  group_by(sim) %>%
  summarize(x = median(x, na.rm = TRUE)) %>%
  geom_density(aes(x), fill = "steelblue", alpha = .5, data = .)

p2 <- my_sims %>%
  geom_density(aes(x), fill = "black", alpha = .5, data = .)
