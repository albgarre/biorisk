...
We define the initial concentration of microorganisms in the product as a uniform distribution over 0-1 log CFU/g (`Uniform` module). As an illustration, it is defined on the variability level (`level = 0`).
...
We will describe microbial growth during storage using the exponential growth model with stationary phase (bilineal model):
...
As above, we first need to define the secondary model. Luckily, **biorisk** includes the `Ratkowsky_model` module that implements this model. It has several inputs (treatment temperature, $T_{min}$ and $b$). For the temperature, we assign a normal distribution that represents uncertainty (`level = 1`) with expected value `6.35` and standard deviation `2.83`. 
...
After that, we define the primary growth model. The package implements the `ExponentialGrowthNmax` model, which is exactly what we need. This modules needs as input the storage time, for which we assume an exponential distribution with rate parameter `1/29`.
...
The next step is to convert the microbial concentration (in log CFU/g) to microbial dose consumed. The package includes the `Concentration2Dose` module to help in this calculation. Note that this module considers the fact that the dose is a sampling process (i.e., the output is a discrete number of cells). In order to obtain continuous values for the dose (e.g., a dose of 0.23 cells), one must use `Concentration2Dose_continous`.

This module has two inputs, the microbial concentration at exposure and the serving size. For the latter, we use a uniform distribution that represents uncertainty (`level = 1`). For the microbial concentration, we map the output of the growth module from above.
...
Then, we can estimate the number of cases. For that, **biorisk** includes the `Pill2Cases_N` module to convert from probability of illness to number of cases. This module considers for each Monte Carlo iteration that the $n_{servings}$ have the same probability of illness. On the other hand, the module `Pill2Cases_1` considers a single serving per $P_{ill}$. In this case, we will make the calculations per $10^{12}$ servings.
